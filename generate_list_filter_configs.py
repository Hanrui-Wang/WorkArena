...
def generate_task_configs(task_class, num_configs=1000, task_type="filter", task_complexity = 2):
    def try_setup_and_cheat(task_class, seed, current_task_configs, task_complexity = 2):
        """Try to setup and cheat a task, and return its configuration if it's new"""
        try:
            with sync_playwright() as p:
                task = task_class(seed=seed)
                task.min_filter_sort_len = task_complexity
                task.max_filter_sort_len = task_complexity
    ...
    with tqdm(total=num_configs, desc=f"Generating {task_name} configs", ncols=150) as pbar:
        while len(current_task_configs) < num_configs:
            seed += 1
            config = try_setup_and_cheat(task_class, seed, current_task_configs, task_complexity)
            if config:
...
if __name__ == "__main__":
    ...
    for task in FILTER_TASKS:
        generate_task_configs(task, num_configs = 10, task_complexity = 2)
